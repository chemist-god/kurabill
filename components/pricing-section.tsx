...
const PricingSection: React.FC = () => {
    const plans: PricingPlan[] = [
        ...
        },
        {
            ...
            buttonColor: 'bg-[#44724A] hover:bg-[#365c3a]',
        },
    ];
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {plans.map((plan, index) => (
                        <motion.div
                            ...
                            className={`relative bg-gray-50 rounded-2xl p-8 ${plan.isPopular ? 'border-2 border-[#44724A]' : ''}`}
                        >
                            {plan.isPopular && (
                            ...
                            <button
                                className={`w-full py-3 px-6 rounded-full text-center transition-all duration-200 
                                ${plan.buttonColor} 
                                ${plan.title === 'Basic' ? 'text-black border border-gray-200' : 'text-white'}`}
                            >
                                Join Waitlist
                            </button>
                        </motion.div>
